...
public class Fetcher extends NutchTool implements Tool,
    ...
    /**
     * 此方法中主要做了这样几件事情：
     * 0. 根据配置初始化 FetchItemQueues 对象
     * 1. 初始化生产者线程 QueueFeeder，并启动线程
     * 2. 根据 threadCount 值初始化消费者线程 FetcherThread，并启动线程
     * 3. 进入do-while循环，循环报告抓取状态，待所有FetcherThread退出，循环退出，run方法结束。
     * @param input
     * @param output
     * @param reporter
     * @throws IOException
