...
public class FetcherThread extends Thread {
    ...
    /**
     * run方法是FetcherThread的核心，这个方法包含具体的下载逻辑。逻辑如下：
     * <p> 1. 进入while死循环，从fetchQueues中获取一个FetchItem对象，FetchItem对象可能为空：
     * <br>     1.1 如果 FetchItem 为 null，检测 QueueFeeder 是否存活或者FetchQueues中是否有FetchItem，
     * <br>         1.1.1 如果 feeder.isAlive() 和 fetchQueues.getTotalSize() > 0 均为false，说明采集任务完成，直接返回。
     * <br>         1.1.2 如果 feeder.isAlive() 或者 fetchQueues.getTotalSize() > 0 其中一个为true，说明整个采集任务还没结束，
     * <br>               当前线程应该进入自旋等待状态(spin-wait)。
     * </p>
     * <p>
     *     2. FetchItem 对象不为空，则进行后续相关操作
     * </p>
     */
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                if (fit == null) {
                    // feeder 继承至 Thread 类，当无更多的 url 需要被转换成 FetchItem 时，feeder 线程生命周期就结束了。
                    ...
                    do {
                        ...
                        if (rules.getCrawlDelay() > 0) {
                            if (rules.getCrawlDelay() > maxCrawlDelay && maxCrawlDelay >= 0) {
                                ...
                                /*
                                 * 如果 rules.getCrawlDelay() 在一个合理值范围内，则fiq的crawlDelay将会被设置为此值。
                                 * FetcherQueue finishFetchItem 方法 最终会调用 setEndTime 方法，用于更新 nextFetchTime 字段，
                                 * 此字段最终会被更新为 nextFetchTime = System.currentTimeMillis + fiq.crawlDelay;
                                 * 当再次调用 FetcherQueue 中的 getFetchItem 方法时，会测试nextFetchTime字段是否大于当前时间，
                                 * 也就意味着这里设置的crawlDelay生效了。
                                 */
    ...
    private void outputRobotsTxt(List<Content> robotsTxtContent) {
        for (Content robotsTxt : robotsTxtContent) {
            ...
            try {
                output.collect(new Text(robotsTxt.getUrl()), new NutchWritable(robotsTxt));
            } catch (IOException e) {
                LOG.error("fetcher caught: {}", e.toString());
            }
        }
